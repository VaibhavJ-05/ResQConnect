...
import { disasterService, campService, sosService, volunteerService, donationService } from '../../services/api';
...
const NGODashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      try {
        const stats = await donationService.getNgoStats();
        setDonationStats(stats);
      } catch (e) {
        console.warn('Failed to load NGO donation stats:', e);
      }
    } catch (error) {
        ...
        <>
          ...
          <Card className="glass-panel border-0 p-4 mb-4">
            ...
            <Tabs defaultActiveKey="assigned" id="ngo-workflow-tabs" className="mb-3">
              ...
              {/* Donation Tab */}
              <Tab eventKey="donations" title="Received Donations">
                <Row className="g-3 mb-4 mt-2">
                  <Col md={4}>
                    <Card className="bg-dark bg-opacity-25 border-secondary text-white p-3">
                      <div className="small text-muted text-uppercase mb-1">Total Donations Received</div>
                      <h3 className="fw-bold text-warning">₹{(donationStats.totalDonations || 0).toFixed(2)}</h3>
                    </Card>
                  </Col>
                  <Col md={4}>
                    <Card className="bg-dark bg-opacity-25 border-secondary text-white p-3">
                      <div className="small text-muted text-uppercase mb-1">Today's Donations</div>
                      <h3 className="fw-bold text-success">₹{(donationStats.todayDonations || 0).toFixed(2)}</h3>
                    </Card>
                  </Col>
                  <Col md={4}>
                    <Card className="bg-dark bg-opacity-25 border-secondary text-white p-3">
                      <div className="small text-muted text-uppercase mb-1">This Month's Donations</div>
                      <h3 className="fw-bold text-info">₹{(donationStats.monthlyDonations || 0).toFixed(2)}</h3>
                    </Card>
                  </Col>
                </Row>

                <h5 className="mb-3 fw-bold text-warning">Recent Donors</h5>
                {(!donationStats.recentDonations || donationStats.recentDonations.length === 0) ? (
                  <Alert variant="info" className="bg-dark border-light border-opacity-10 text-muted py-3 text-center">
                  ...
                  <div className="table-responsive">
                    <Table hover variant="dark" className="align-middle bg-transparent mb-0">
                      ...
                      <tbody>
                        {donationStats.recentDonations.map((d) => (
                          <tr key={d.id} className="border-light border-opacity-10">
                            ...
                            <td className="text-end fw-bold text-warning">₹{d.amount.toFixed(2)}</td>
                            <td className="small text-muted fst-italic">{d.message || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </Table>
