...
import { FiAlertTriangle, FiPlus, FiEdit2, FiTrash2, FiActivity, FiMapPin } from 'react-icons/fi';
...
const formatDateToLocalInput = (dateVal) => {
  if (!dateVal) return '';
...
const GovDisasters = () => {
  ...
  const handleOpenEdit = (disaster) => {
    ...
    setFormData({
      ...
      startDate: formatDateToLocalInput(disaster.startDate),
      endDate: formatDateToLocalInput(disaster.endDate),
    });
    setShowModal(true);
  };
  ...
  return (
    <Container className="py-4 text-start">
      ...
      <Modal show={showMapPicker} onHide={() => setShowMapPicker(false)} contentClassName="glass-panel text-light" dialogClassName="modal-dark" size="lg" centered>
        <Modal.Header closeButton closeVariant="white" className="border-light border-opacity-10 bg-dark">
          <Modal.Title className="fw-bold d-flex align-items-center gap-2"><FiMapPin /> Select Location on Map</Modal.Title>
        </Modal.Header>
        <Modal.Body className="bg-dark p-2 text-start">
          <div style={{ height: '400px', width: '100%', borderRadius: '8px', overflow: 'hidden' }}>
            {showMapPicker && tempLocation && (
              <MapContainer 
                center={[tempLocation.lat, tempLocation.lng]} 
                zoom={tempLocation.lat !== 20.5937 ? 12 : 5} 
          ...
          {tempLocation && (
            <div className="mt-3 text-muted small px-2">
              Selected Coordinates: <strong>{tempLocation.lat.toFixed(6)}</strong>, <strong>{tempLocation.lng.toFixed(6)}</strong>
            </div>
          )}
          <div className="d-flex justify-content-end gap-2 mt-3 px-2 pb-2">
            <Button variant="outline-light" onClick={() => setShowMapPicker(false)}>Cancel</Button>
            <Button variant="premium" onClick={handleConfirmLocation} disabled={!tempLocation}>
              Confirm Location
            </Button>
          </div>
        </Modal.Body>
